...
function greedy_merge_index{T<:FloatingPoint,S<:Integer}(
        ...
        MODL = log(n) + lfact(2n-1) - lfact(n-1) - lfact(n) + n*log(J-1)
...
function methods_split(continuous,class_value,distr,i,j,uniq_class,I)
        ...
        for n_A_end = 1:n-1
                ...
                for j = 1:J
                        Del += lfact(distr_A[j]+distr_B[j])-lfact(distr_A[j])-lfact(distr_B[j])
                end
...
function PQ_methods_updata(continuous,class_value,uniq_class,binedges,distr,post_pq,effects,index,method)
        ...
        elseif method == "S"
                ...
                effects[(binedges[index],method)] = method_result[2:]
        elseif method == "MS"
...
function post_greedy_index(continuous,class_value,upper_bound = 0)
        ...
        for i = 1 : I-2
                ...
                effects[(bins[i],"S")] = method_result[2:]
                ...
                effects[(bins[i],"MS")] = method_result[2:]
                ...
                effects[(bins[i],"MMS")] = method_result[2:]
        ...
        if I>1
                ...
                effects[(bins[I-1],"S")] = method_result[2:]
                ...
                effects[(bins[I-1],"MS")] = method_result[2:]
...
type DiscretizeMODL <: DiscretizationAlgorithm
        version :: Symbol #:optimal, :greedy, :post_greedy
end

optimal = DiscretizeMODL(:optimal)
greedy = DiscretizeMODL(:greedy)
post_greedy = DiscretizeMODL(:post_greedy)

function binedges{N<:Real, I<:Integer}(
        alg                :: DiscretizeMODL,
        data_continuous    :: AbstractArray{N},
        data_class         :: AbstractArray{I},
        max_bin_number:: Int64 =0)
        ...
        if alg.version == :optimal
                return optimal_result(data_continuous,data_class)
        elseif alg.version == :greedy
                return greedy_merge(data_continuous,data_class)
        elseif alg.version == :post_greedy
                return post_greedy_result(data_continuous,data_class,max_bin_number)
        else
